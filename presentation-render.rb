...
for get in ['shapes.svg', 'video/webcams.webm', 'deskshare/deskshare.webm'] do
    download(get)
end
...
system("ffmpeg -i deskshare/deskshare.webm -c:v libvpx-vp9 -i presentation.webm -c:v libvpx-vp9 -i whiteboard.webm -i video/webcams.webm -filter_complex '[3]scale=w=iw/4:h=ih/4[cam];[0][1]overlay[tmp];[tmp][2]overlay[out];[out][cam]overlay=x=(main_w-overlay_w)' -b:v 2M -shortest -y presentation-deskshare-webcam.webm")
